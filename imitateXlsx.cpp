...
//    format_set_font_name(header, "Times New Roman");
    ...
    worksheet_set_column_opt(ws, 5, 120, 16.14, NULL, NULL);
...
//    worksheet_set_row_opt(ws, 2, 17.25, NULL, NULL);
    ...
    for(int i = 0; i <= n; i++)
    {
        ...
        worksheet_write_number(ws, 3+i, 54, mlm->heterogeneousSlip(vtemp), clnum);
        worksheet_write_number(ws, 3+i, 55, mlm->aroundLDVSlip(vtemp), clnum);
        worksheet_write_number(ws, 3+i, 56, mlm->fixedBedSlip(vtemp), clnum);
        worksheet_write_number(ws, 3+i, 57, mlm->threeLMSlip(vtemp), clnum);
        worksheet_write_number(ws, 3+i, 58, mlm->slipTangent(vtemp), clnum);
        worksheet_write_number(ws, 3+i, 59, mlm->slipRatio(vtemp), clnum);
        worksheet_write_number(ws, 3+i, 60, mlm->headLossCvt(vtemp), clnum);
    ...
    worksheet_insert_chart_opt(ws,CELL("E3"),cl_chart, &ch_options);
    ...
    chart_axis_set_min(sr_chart->y_axis, 0.0);
    chart_axis_set_max(sr_chart->y_axis, 1.0);

    chart_axis_major_gridlines_set_visible(sr_chart->x_axis, LXW_TRUE);
    chart_axis_set_minor_unit(sr_chart->x_axis, 0.2);
    chart_axis_minor_gridlines_set_visible(sr_chart->x_axis, LXW_TRUE);
    chart_axis_set_minor_unit(sr_chart->y_axis, 0.02);
    chart_axis_minor_gridlines_set_visible(sr_chart->y_axis, LXW_TRUE);

    chart_axis_set_name(sr_chart->x_axis, "Line speed [m/s]");
    chart_axis_set_name(sr_chart->y_axis, "Slip ratio [-]");

    chart_title_set_name(sr_chart, "Carrier liquid based slip ratios");
    chart_title_set_name_font(sr_chart, &font);
    ...
    worksheet_insert_chart_opt(ws,CELL("E174"),sr_chart, &ch_options);
